...
export default function Profile() {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (file) {
        ...
        const storageRef = ref(storage, `profiles/${storedId}-${Date.now()}`);
      ...
      const res = await fetch(`/api/user/update/${storedId}`, {
        method: 'POST',
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/user/delete/${storedId}`, {
        method: 'DELETE',
        credentials: 'include',
      });
      if (res.ok) {
